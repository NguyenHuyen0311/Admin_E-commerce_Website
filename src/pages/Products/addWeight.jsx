...
const AddWeight = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (name === "") {
      context.openAlertBox("error", "Vui lòng nhập cân nặng sản phẩm!");
      setIsLoading(false);
      return false;
    }
  ...
  const deleteWeight = (id) => {
    deleteData(`/api/product/productWeight/${id}`).then((res) => {
      getData();
      context.openAlertBox("success", "Xóa cân nặng sản phẩm thành công!");
    });
  };
  ...
  return (
    ...
    >
      <div className="w-full px-5 flex justify-between mb-3 items-center">
        <h2 className="text-[18px] font-bold">Thêm cân nặng sản phẩm</h2>
      </div>
